...
| **Memory poisoning / security-sensitive memory policy (P0)** | An instruction hidden in pasted material (email, web page, log) that would come back as trusted memory in every later prompt. OpenWiki never persists instructions to AI assistants, security weakening, secrets/payments directed somewhere or standing authorizations — regardless of the fact's `source` tag, which injections launder (ADR-30). |
| **Provenance (`source`)** | Who put a fact into the conversation: `user` · `assistant` · `material` (discussed documents). A *soft* signal only — "Material" marker, ×0.75 recall weight (ADR-30). |
| **Sleep / forgotten** | `openwiki sleep` — the nightly, schedulable memory maintenance pass: fold queued writes, **forget**, re-consolidate themes, decay usage edges. A **forgotten** fact (*vergessen*) was archived by policy — a one-off session event ("vX was pushed and tagged"), a commit hash, a tautology, or a P0-unsafe fact: out of recall and context, kept in the graph, visible to earlier `known_at` views (ADR-32). |
| **Cue-trigger recall / constraint probe (P1)** | Recalling a constraint the user mentioned in passing ("can't stand noisy offices") for a later request that shares no words with it ("book a venue"). A **constraint probe** is a hypothetical user fact ("user cannot stand noise") one chat call guesses for the request; it reserves a recall slot for a fact *about the user*, shown first under "Keep in mind". Opt-in via `[memory] probes` (ADR-31). |
...
— ADR-27), memory hygiene (poisoning, provenance — ADR-30), cue-trigger recall (constraint probes — ADR-31) and forgetting (sleep, forgotten — ADR-32).*

"""Kuzu-backed knowledge-graph layer over the wiki (structural + vector-derived)."""

from .builder import GraphBuilder, build_graph
from .community import answer_global, detect_communities, summarize_community, summarize_facts
from .entities import (
    DEFAULT_ENTITY_TYPES, Entity, Relation, coerce_types, extract_entities, extract_relations,
    resolve_entities,
)
from .memory import (
    MemoryFact, assemble_context, capture_session, capture_session_detailed, choose_attribute,
    facts_coexist, flag_injected, format_memory, is_unsafe_instruction, parse_facts,
)
from .references import detect_page_offset, extract_references, extract_references_multi
from .store import GraphStore
from .usage import append_usage, clear_usage, read_usage, usage_log_path

__all__ = [
    "GraphBuilder", "build_graph", "GraphStore",
    "extract_references", "extract_references_multi", "detect_page_offset",
    "extract_entities", "extract_relations", "resolve_entities", "Entity", "Relation",
    "coerce_types", "DEFAULT_ENTITY_TYPES",
    "detect_communities", "summarize_community", "summarize_facts", "answer_global",
    "capture_session", "format_memory", "assemble_context", "parse_facts", "MemoryFact",
    "facts_coexist", "choose_attribute", "capture_session_detailed", "flag_injected",
    "is_unsafe_instruction",
    "usage_log_path", "append_usage", "read_usage", "clear_usage",
]

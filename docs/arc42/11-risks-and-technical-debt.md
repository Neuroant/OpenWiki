...
| R8 | **LLM-dependent memory semantics** — the capture model decides a fact's stated date + cardinality, and a coexistence verdict can keep a stale value current; and (since the hooks inject memory into every prompt) **memory poisoning** — an instruction hidden in pasted material could come back as trusted memory | Medium (wrong current fact; injected instruction) | Low–Medium | Dates are taken only when *stated* (never guessed) and validated; the coexistence check is **veto-only** (it can keep a fact, never hide one), deterministic (temperature 0), and skipped for explicit `--correct`; measured on `eval_temporal.jsonl` (ADR-27). Poisoning: a source-independent security-sensitive memory policy (ADR-30) — `eval_poisoning.jsonl` 2/5 → 0/5 leaked, legit facts kept. |
